...
- **`trips.json`** — 구조화된 기록(gitignore됨)으로, 이후 블로그 초안 생성기의 입력이 됩니다.
...
- **Phase 2 — 블로그 초안.** `trips.json`은 다음 단계의 계약으로 설계됐습니다: 각 여행(과
  대표 사진)을 당신의 말투로 블로그 초안으로 바꾸는 단계. JSON은 이미 필요한 것을 다 담고
  있습니다(재현지화 가능한 장소 이름, 날짜별 구조, 사진 UUID, 신뢰 신호).
